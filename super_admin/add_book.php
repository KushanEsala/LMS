    ...
    if (in_array($book_image_ext, $allowed)) {
        if ($book_image_error === 0) {
            if ($book_image_size < 5000000) { // 5 MB limit
                ...
                $book_image_destination = 'uploads' . $book_image_name_new;
                if (move_uploaded_file($book_image_tmp_name, $book_image_destination)) {
                    ...
                    // Bind parameters
                    mysqli_stmt_bind_param($stmt, "ssisiiis", $book_name, $book_author, $book_category, $isbn_no, $book_price, $book_quantity, $book_availability, $book_image_name_new);
                    ...
                    // Check for success
                    if ($success) {
                        echo '<script>window.onload = function() { alert("Book added successfully."); window.location.href = "add_book.php"; }</script>';
                    } else {
